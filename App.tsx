...
function App() {
  ...
  // MANDATORY: Set initial view to 'auth' to force login on refresh
  ...
  const handleViewChange = (view: View) => {
    // ROUTE PROTECTION: If user isn't logged in, they can only stay in 'auth' or 'showcase' (optional)
    // Here we strictly enforce 'auth' unless user exists
    if (!user && view !== 'auth' && view !== 'showcase') {
      console.log(`[Auth Guard] Blocked access to ${view}. Redirecting to login.`);
      setCurrentView('auth');
      return;
    }
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${getMainBackground()} ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <div className="flex-1 flex flex-col">
        {/* If no user, only allow Auth or Showcase */}
        {!user && currentView !== 'showcase' ? (
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                  ...
                  <div className="w-full lg:w-[400px] bg-white dark:bg-slate-900 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-gray-100 dark:border-slate-800 p-6 flex flex-col gap-6 shrink-0 transition-all">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input 
                        type="text"
                        className="w-full border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 text-slate-900 dark:text-white rounded-2xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none placeholder-gray-400 dark:placeholder-slate-600 transition-all"
